...
router.get('/', async (req, res, next) => {
  try {
    res.json(await POAM.find().sort({ _id: 1 }));
  } catch (err) { next(err); }
});
...
router.post('/', async (req, res, next) => {
  const { title, control_id, weakness, severity, status, site, source_type, source_id,
...
router.patch('/:id', async (req, res, next) => {
  ...
  try {
    const doc = await POAM.findByIdAndUpdate(req.params.id, { $set: updates }, { new: true });
    if (!doc) return res.status(404).json({ error: 'Not found' });
    ...
    await audit(req.session.user.username, 'POAM_UPDATE', req.params.id, `Updated: ${Object.keys(updates).join(', ')}`, doc.site);
    res.json(doc);
  } catch (err) { next(err); }
});
...
router.delete('/:id', async (req, res, next) => {
  try {
    const doc = await POAM.findByIdAndDelete(req.params.id);
    if (!doc) return res.status(404).json({ error: 'Not found' });
